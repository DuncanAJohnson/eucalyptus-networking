...
URL_RE = re.compile(r'wss://([A-Za-z0-9\-\.]+)(?:\:([0-9]+))?(/.+)?')
...
def urlparse(uri):
       ...
       if match:
           ...
           port = int(match.group(2)) if match.group(2) else 80  # Default to port 80
...
def connect(uri):
    ...
    send_header(b'GET %s HTTPS/1.1', uri.path or '/')
    ...
    assert header == b'HTTPS/1.1 101 Switching Protocols', header
